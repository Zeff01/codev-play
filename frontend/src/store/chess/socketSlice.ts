...
export const createSocketSlice: StateCreator<ChessStore, [], [], SocketSlice> = (set, get) => ({
    ...
    setSocket: (socket) => {
        ...
        socket.on("chess:gameStart", (data: { playerColor: Color; timeControl: number; gameId: string }) => {
            socket.emit("game:join", { gameId: data.gameId, gameType: "chess" });
        ...
        socket.on("game:move", (data: { gameId: string; game: any; moveData: any }) => {
            const game = data.game;
            if (!game) return;

            const { playerColor } = get();
            const movingColor: Color = game.move?.color ?? (game.current_turn === "w" ? "b" : "w");
            ...
            const status: GameStatus = statusMap[game.status] ?? "playing";
            const finalStatus = game.is_check && status === "playing" ? "check" : status;
        ...
        socket.on("chess:draw", (data: { drawStatus: string; offeredBy: number; status: string }) => {
