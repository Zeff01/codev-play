    ...
    <div className='menu-bar fixed top-0 left-0 w-full z-20 -translate-y-20'>
      <div className='max-w-450 w-full mx-auto px-8 py-4 flex justify-between items-center text-white'>
        ...
        <div className="menu-open hidden md:block">
          ...
          {/* Sidebar */}
        <div ref={overlayRef} className="fixed top-0 right-0 h-dvh w-full sm:w-62.5 z-20 shadow-lg bg-black/10 backdrop-blur-xl translate-x-256">
          <div className='p-6 flex justify-between items-center'>
            <Link href="/">
                <Image src={CodevPlay} alt="CodevPlay Logo" className='object-contain w-42 sm:w-32 h-12'/>
            </Link>
          ...
          {/* optimized: make each anchor a block that fills the container so hover covers full width */}
            ...
            <li><Button onClick={() => router.push('/login')} className='mx-4 my-4 px-8 text-lg cursor-pointer hover:bg-[#9747FF] hover:text-white'>Sign In</Button></li>
            </ul>
        </div>
      </div>
    </div>
